...
class ProTalksCore_Titles
{
    ...
    public function __construct()
    {
        ...
        add_filter('protalks_filter_page_title_classes', array( $this, 'add_page_title_classes' ));
    ...
    public function include_layouts()
    {

        foreach (glob(PROTALKS_CORE_INC_PATH . '/title/dashboard/*/*.php') as $admin) {
            include_once $admin;
        }

        foreach (glob(PROTALKS_CORE_INC_PATH . '/title/layouts/*/include.php') as $layout) {
            include_once $layout;
        }
    }
    ...
    {
        $layout_meta = protalks_core_get_post_value_through_levels('qodef_title_layout');
        $layouts     = apply_filters('protalks_core_filter_register_title_layouts', array());
    ...
    public function render_template()
    {
        ...
        if (! empty($title_object)) {
            $template_hook = $title_object->overriding_whole_title ? 'protalks_filter_title_template' : 'protalks_filter_title_content_template';
    ...
    public function add_page_title_classes($classes)
    {
        $layout             = protalks_core_get_post_value_through_levels('qodef_title_layout');
        $text_alignment     = protalks_core_get_post_value_through_levels('qodef_page_title_text_alignment');
        $vertical_alignment = protalks_core_get_post_value_through_levels('qodef_page_title_vertical_text_alignment');
    ...
    public function add_inline_styles($style)
    {
        ...
        $height                   = protalks_core_get_post_value_through_levels('qodef_page_title_height');
        $height_mobile            = protalks_core_get_post_value_through_levels('qodef_page_title_height_on_smaller_screens');
        $title_padding            = apply_filters('protalks_core_filter_title_padding', 0);
        $title_padding_mobile     = apply_filters('protalks_core_filter_title_padding_mobile', 0);
        $title_vertical_alignment = protalks_core_get_post_value_through_levels('qodef_page_title_vertical_text_alignment');
        $background_color         = protalks_core_get_post_value_through_levels('qodef_page_title_background_color');
        $image_params             = protalks_core_get_page_title_image_params();
        ...
        $title_color = protalks_core_get_post_value_through_levels('qodef_page_title_color');
...
ProTalksCore_Titles::get_instance();
