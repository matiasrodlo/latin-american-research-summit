<div <?php qode_framework_class_attribute( $holder_classes ); ?> <?php qode_framework_inline_style( $holder_styles ); ?> <?php qode_framework_inline_attr( $data_attr, 'data-options' ); ?>>
	<?php
	// Include global filter from theme.
	protalks_core_theme_template_part( 'filter', 'templates/filter', '', $params );
	?>
	<ul class="qodef-grid-inner">
		<?php
		// Include global masonry template from theme.
		protalks_core_theme_template_part( 'masonry', 'templates/sizer-gutter', '', $params['behavior'] . '-product-list' );

		// Include items.
		protalks_core_template_part( 'plugins/woocommerce/shortcodes/product-list', 'templates/loop', '', $params );
		?>
	</ul>
	<?php
	// Include global pagination from theme.
	protalks_core_theme_template_part( 'pagination', 'templates/pagination', $params['pagination_type'], $params );
	?>
</div>
