...
class ProTalksCore_Headers {
	...
	public function __construct() {
		...
		add_action( 'protalks_action_after_page_header_inner', array( $this, 'scroll_appearance' ) );
	...
	public function include_elements() {
		...
		foreach ( glob( PROTALKS_CORE_INC_PATH . '/header/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( PROTALKS_CORE_INC_PATH . '/header/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}

		foreach ( glob( PROTALKS_CORE_INC_PATH . '/header/*/include.php' ) as $header_part ) {
			include_once $header_part;
		}
	}

	public function set_variables() {
		$layout_meta = protalks_core_get_post_value_through_levels( 'qodef_header_layout' );
		$layouts     = apply_filters( 'protalks_core_filter_register_header_layouts', array() );
	...
	public function render_template() {
		...
		if ( ! empty( $header_object ) ) {
			$template_hook = $header_object->is_whole_header_override() ? 'protalks_filter_header_template' : 'protalks_filter_header_content_template';
	...
	public function scroll_appearance() {
		$header_object = $this->get_header_object();

		if ( ! empty( $header_object ) ) {
			$appearance_type = protalks_core_get_post_value_through_levels( 'qodef_header_scroll_appearance' );

			if ( file_exists( PROTALKS_CORE_HEADER_LAYOUTS_PATH . '/' . $header_object->get_layout() . '/templates/' . $appearance_type . '.php' ) ) {
				$scroll_appearance_layout = 'layouts/' . $header_object->get_layout();
			} else {
				$scroll_appearance_layout = 'scroll-appearance/' . $appearance_type;
			}

			protalks_core_template_part( 'header/' . $scroll_appearance_layout, 'templates/' . $appearance_type, $header_object->get_layout_slug(), array() );
		}
	}
	...
	public function add_header_widget_areas() {
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area One', 'protalks-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area one', 'protalks-core' ),
		...
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area Two', 'protalks-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area two', 'protalks-core' ),
		...
		do_action( 'protalks_core_action_additional_header_widgets_area' );
	}
}

ProTalksCore_Headers::get_instance();
