...
if ( ! function_exists( 'protalks_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function protalks_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'ProTalksCore_Highlight_Shortcode';

		return $shortcodes;
	}

	add_filter( 'protalks_core_filter_register_shortcodes', 'protalks_core_add_highlight_shortcode' );
}

if ( class_exists( 'ProTalksCore_Shortcode' ) ) {
	class ProTalksCore_Highlight_Shortcode extends ProTalksCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( PROTALKS_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'protalks_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'protalks-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'protalks-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Disable Text Line Break', 'protalks-core' ),
					'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1200 and lower', 'protalks-core' ),
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Highlight Text Position', 'protalks-core' ),
					'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'protalks-core' ),
					'group'       => esc_html__( 'Text Style', 'protalks-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Color', 'protalks-core' ),
					'group'      => esc_html__( 'Text Style', 'protalks-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Background Color', 'protalks-core' ),
					'group'      => esc_html__( 'Text Style', 'protalks-core' ),
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return protalks_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
