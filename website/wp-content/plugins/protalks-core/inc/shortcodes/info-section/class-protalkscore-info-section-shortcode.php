...
if ( ! function_exists( 'protalks_core_add_info_section_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function protalks_core_add_info_section_shortcode( $shortcodes ) {
		$shortcodes[] = 'ProTalksCore_Info_Section_Shortcode';

		return $shortcodes;
	}

	add_filter( 'protalks_core_filter_register_shortcodes', 'protalks_core_add_info_section_shortcode' );
}

if ( class_exists( 'ProTalksCore_Shortcode' ) ) {
	class ProTalksCore_Info_Section_Shortcode extends ProTalksCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'protalks_core_filter_info_section_layouts', array() ) );

			$options_map   = protalks_core_get_variations_options_map( $this->get_layouts() );
			$default_value = $options_map['default_value'];

			$this->set_extra_options( apply_filters( 'protalks_core_filter_info_section_extra_options', array(), $default_value ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( PROTALKS_CORE_SHORTCODES_URL_PATH . '/info-section' );
			$this->set_base( 'protalks_core_info_section' );
			$this->set_name( esc_html__( 'Info Section', 'protalks-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds info section element', 'protalks-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
		...
		public function render( $options, $content = null ) {
			...
			$atts = apply_filters( 'protalks_core_filter_info_section_variation_atts', $atts );

			return protalks_core_get_template_part( 'shortcodes/info-section', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes   = apply_filters( 'protalks_core_filter_info_section_variation_classes', $holder_classes, $atts );
