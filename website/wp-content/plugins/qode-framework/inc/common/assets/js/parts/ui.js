(function ( $ ) {
	...
	var qodefAdminOptionsPanel = {
		...
		getCookie: function ( name ) {
			...
			var cookies_count = cookieArray.length;
			for ( var i = 0; i < cookies_count; i++ ) {
		...
		multipleSelect: function ( varialbles, $holder ) {
			varialbles.mediaFrame.on(
				...
				function () {
					...
					var attachments_count = varialbles.attachment.length;
					for ( var i = 0; i < attachments_count; ++i ) {
	...
	var qodefPostFormatsDependency = {
		...
		classicEditor: function () {
			...
			if ( $holder.length ) {
				var $postFormats     = $holder.find( 'input[name="post_format"]' ),
	...
	var qodefAddressFields = {
		...
		initMap: function ( $holder, trigger ) {
			var $reset       = $holder.find( '.qodef-reset-marker' ),
				...
				initialLat   = parseFloat( $latField.val() ),
				initialLng   = parseFloat( $lngField.val() );

			if ( initialLat && initialLng ) {
				var initialLocation = new google.maps.LatLng( initialLat, initialLng );
			...
			if ( typeof google.maps.places === 'object' && typeof trigger === 'undefined' ) {
				var autocomplete_args = {
					types: ['geocode', 'establishment'],
				};

				if ( typeof countryLimit !== 'undefined' && '' !== countryLimit ) {
					autocomplete_args.componentRestrictions = {
						country: countryLimit,
					}
				}

				// Initialize Google Places Autocomplete.
				var autocomplete = new google.maps.places.Autocomplete( $inputField[0], autocomplete_args );

				// Set up map when address is selected from the autocomplete suggestions.
				autocomplete.addListener(
					...
					function () {
						...
						// If a place is found, set the marker and address details.
						...
						// Set the input field value to the formatted address.
						$inputField.val( place.formatted_address );
					}
				);
			}

			// Handle resetting the address input and fields.
			$reset.on(
				'click',
				function ( e ) {
					e.preventDefault();
