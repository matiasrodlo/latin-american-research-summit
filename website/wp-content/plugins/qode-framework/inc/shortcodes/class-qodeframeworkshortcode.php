		...
		if ( 'repeater' === $params['field_type'] ) {
			foreach ( $params['items'] as $item_key => $item ) {
				...
				if ( 'iconpack' === $item['field_type'] ) {
					...
					$icon_packs_with_default = $icons_object->get_icon_packs( array(), true );

					if ( is_array( $icon_packs ) && ! empty( $icon_packs ) ) {
						$item['options']              = $icon_packs_with_default;
						$params['items'][ $item_key ] = $item;

						foreach ( $icon_packs as $icon_pack_key => $icon_pack_name ) {
							$icon_field_name = qode_framework_icons()->get_formatted_icon_field_name( $item['name'], $icon_pack_key );
			...
			if ( is_array( $icon_packs ) && ! empty( $icon_packs ) ) {
				$params['options']     = $icon_packs_with_default;
				$this->options[ $key ] = $params;

				foreach ( $icon_packs as $icon_pack_key => $icon_pack_name ) {
					$icon_field_name = qode_framework_icons()->get_formatted_icon_field_name( $key, $icon_pack_key );
