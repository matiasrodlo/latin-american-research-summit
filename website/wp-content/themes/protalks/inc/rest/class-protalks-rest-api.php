...
if ( ! class_exists( 'Protalks_Rest_API' ) ) {
	/**
	 * Rest API class with configuration
	 */
	class Protalks_Rest_API {
		private static $instance;
		private $namespace;

		public function __construct() {
			// Init variables.
			$this->set_namespace( 'protalks/v1' );

			// Localize theme's main js script with rest variable.
			add_filter( 'protalks_filter_localize_main_js', array( $this, 'localize_script' ) );
		...
		public function localize_script( $global ) {
			...
			return apply_filters( 'protalks_filter_rest_api_global_variables', $global, $this->get_namespace() );
		}

		public function register_rest_api_route() {
			$routes = apply_filters( 'protalks_filter_rest_api_routes', array() );
	...
	Protalks_Rest_API::get_instance();
}
