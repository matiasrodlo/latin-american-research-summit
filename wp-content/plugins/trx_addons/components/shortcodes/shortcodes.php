...
if ( !function_exists( 'trx_addons_ajax_sc_pagination' ) ) {
	...
	function trx_addons_ajax_sc_pagination() {
		...
		if ( ! empty( $_POST['params'] ) && is_string( $_POST['params'] ) ) {
			...
			if ( preg_match( '/O:[0-9]+:"([^"]*)":[0-9]+:{/i', $_POST['params'], $matches ) ) {
				$_POST['params'] = '[]';
			}
			$params = substr( $_POST['params'], 0, 2 ) == 'a:'
						? trx_addons_unserialize( wp_unslash( $_POST['params'] ) )	// Old format (params are serialized - if an old template 'templates/tpl.sc_pagination.php' copied to the theme/skin)
