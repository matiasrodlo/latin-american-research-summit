		...
		if ( 'iconpack' === $params['field_type'] ) {
			...
			if ( is_array( $icon_packs ) && ! empty( $icon_packs ) ) {
				foreach ( $icon_packs as $icon_pack_key => $icon_pack_name ) {
					$icon_field_name = $icons_object->get_formatted_icon_field_name( $key, $icon_pack_key );
		...
		if ( is_array( $this->widget_options ) && count( $this->widget_options ) ) { ?>
			...
			foreach ( $this->widget_options as $option ) {
				...
				if ( ! empty( $option['title'] ) || ( isset( $option['name'] ) && ! empty( $option['name'] ) ) ) {
					...
					$class   = implode( ' ', $class );
					?>
					<p class="<?php echo esc_attr( $class ); ?>" <?php qode_framework_inline_attrs( $dependency_data, true ); ?>>
						...
						<?php if ( ! empty( $option['description'] ) ) : ?>
							<span class="qodef-field-description"><?php echo wp_kses_post( $option['description'] ); ?></span>
						<?php endif; ?>
					</p>
				<?php } ?>
			<?php } ?>
