...
class QodeFrameworkElementor_Translator {
	...
	public function generate_options_array( $option_key, $option ) {
		...
		if ( ! isset( $visibility['map_for_page_builder'] ) || ( isset( $visibility['map_for_page_builder'] ) && true === $visibility['map_for_page_builder'] ) ) {
			...
			if ( isset( $option['min'] ) ) {
				$formatted_options[ $group ]['fields'][ $option_key ]['min'] = $option['min'];
			}
			if ( isset( $option['max'] ) ) {
				$formatted_options[ $group ]['fields'][ $option_key ]['max'] = $option['max'];
			}
	...
	/**
	 * Function that set our category for Elementor widgets.
	 *
