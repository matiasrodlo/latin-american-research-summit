...
class QodeFrameworkIcons {
	...
	public $icon_pack_icons = array();

	public function __construct() {
		$this->register_icon_packs();

		add_action( 'qode_framework_before_dashboard_scripts', array( $this, 'enqueue_style' ) );
		add_action( 'qode_framework_action_before_dashboard_widgets_scripts', array( $this, 'enqueue_style' ) );
	...
	public function set_icons_collection( QodeFrameworkIconPack $icon_pack ) {
		...
		$this->set_icon_pack_icons( $key, $icon_pack->get_icons() );
	...
	public function get_icon_pack_icons( $icon_pack ) {
		return $this->icon_pack_icons[ $icon_pack ] ?? array();
	}

	public function set_icon_pack_icons( $key, $icons ) {
		$this->icon_pack_icons[ $key ] = $icons;
	}
		...
		if ( ! empty( $icons ) ) {
			foreach ( $icons as $icon ) {
				if ( class_exists( $icon ) ) {
					$this->set_icons_collection( call_user_func( array( $icon, 'get_instance' ) ) );
				}
			}
		}
	}
	...
	public function enqueue_style() {
		$icon_objects = $this->get_icon_object_collection();

		if ( is_array( $icon_objects ) && ! empty( $icon_objects ) ) {
			foreach ( $icon_objects as $collection_key => $collection_obj ) {
				$collection_key = apply_filters( 'qode_framework_filter_icon_pack_enqueue_handle', $collection_key );

				wp_enqueue_style( $collection_key, $collection_obj->get_style_url(), (array) apply_filters( 'qode_framework_filter_add_icon_dependencies', array() ), QODE_FRAMEWORK_VERSION );
			}
		}
	}
