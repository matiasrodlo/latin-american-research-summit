...
if ( ! function_exists( 'protalks_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function protalks_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'ProTalksCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'protalks_core_filter_register_shortcodes', 'protalks_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'ProTalksCore_Shortcode' ) ) {
	class ProTalksCore_Icon_Shortcode extends ProTalksCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( PROTALKS_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'protalks_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'protalks-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'protalks-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'tiny'    => esc_html__( 'Tiny', 'protalks-core' ),
						'small'   => esc_html__( 'Small', 'protalks-core' ),
						'medium'  => esc_html__( 'Medium', 'protalks-core' ),
						'large'   => esc_html__( 'Large', 'protalks-core' ),
						'huge'    => esc_html__( 'Huge', 'protalks-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'protalks-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'options'       => array(
						'normal' => esc_html__( 'Normal', 'protalks-core' ),
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'protalks-core' ),
					'group'      => esc_html__( 'Style', 'protalks-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'protalks-core' ),
					'group'      => esc_html__( 'Style', 'protalks-core' ),
				)
			);
			$this->set_option(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'protalks_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return protalks_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
