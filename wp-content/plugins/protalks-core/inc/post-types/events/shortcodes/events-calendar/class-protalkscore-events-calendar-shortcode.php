...
if ( ! function_exists( 'protalks_core_add_events_calendar_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function protalks_core_add_events_calendar_shortcode( $shortcodes ) {
		$shortcodes[] = 'ProTalksCore_Events_Calendar_Shortcode';

		return $shortcodes;
	}

	add_filter( 'protalks_core_filter_register_shortcodes', 'protalks_core_add_events_calendar_shortcode' );
}

if ( class_exists( 'ProTalksCore_List_Shortcode' ) ) {
	class ProTalksCore_Events_Calendar_Shortcode extends ProTalksCore_List_Shortcode {

		public function __construct() {
			$this->set_post_type( 'event-item' );
			$this->set_post_type_additional_taxonomies( array( 'event-types' ) );
			$this->set_layouts( apply_filters( 'protalks_core_filter_events_calendar_layouts', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( PROTALKS_CORE_CPT_URL_PATH . '/events/shortcodes/events-calendar' );
			$this->set_base( 'protalks_core_events_calendar' );
			$this->set_name( esc_html__( 'Events Calendar', 'protalks-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays calendar of events', 'protalks-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'protalks_core_events_calendar', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			$atts['data_attr']      = protalks_core_get_pagination_data( PROTALKS_CORE_REL_PATH, 'post-types/events/shortcodes', 'events-calendar', 'events', $atts );

			$atts['this_shortcode'] = $this;

			// needed to exist in order to have it checked with date of current event in loop
			$atts['current_date'] = '';

			return protalks_core_get_template_part( 'post-types/events/shortcodes/events-calendar', 'templates/content', $atts['behavior'], $atts );
		}

		private function get_holder_classes( $atts ) {
			$holder_classes = $this->init_holder_classes();

			$holder_classes[] = 'qodef-events-calendar';
