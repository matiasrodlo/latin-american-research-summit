...
if ( ! function_exists( 'protalks_core_include_role_custom_fields' ) ) {
	/**
	 * Function that includes role custom fields files
	 */
	function protalks_core_include_role_custom_fields() {
		foreach ( glob( PROTALKS_CORE_INC_PATH . '/roles/*/role-fields.php' ) as $role_fields ) {
			include_once $role_fields;
		}
	}

	add_action( 'qode_framework_action_custom_user_fields', 'protalks_core_include_role_custom_fields' );
}

if ( ! function_exists( 'protalks_core_register_role_custom_fields' ) ) {
	/**
	 * Function that registers role custom fields files
	 */
	function protalks_core_register_role_custom_fields() {
		do_action( 'protalks_core_action_register_role_custom_fields' );
	}

	add_action( 'qode_framework_action_custom_user_fields', 'protalks_core_register_role_custom_fields', 11 );
}

if ( ! function_exists( 'protalks_core_profile_get_social_params' ) ) {
	/**
	 * Returns owner params
	 */
	function protalks_core_profile_get_social_params( $params ) {
	...
	add_filter( 'protalks_membership_filter_user_params', 'protalks_core_profile_get_social_params' );
}

if ( ! function_exists( 'protalks_core_social_edit_profile_fields' ) ) {
	function protalks_core_social_edit_profile_fields( $page, $params ) {
		$roles_social_scope = apply_filters( 'protalks_core_filter_role_social_array', array( 'administrator', 'author' ) );
		$enable             = false;
	...
	add_action( 'protalks_membership_action_after_dashboard_edit_profile_fields', 'protalks_core_social_edit_profile_fields', 15, 2 );
}

if ( ! function_exists( 'protalks_core_user_profile_social_update' ) ) {
	function protalks_core_user_profile_social_update( $options, $user_id ) {
	...
	add_action( 'protalks_membership_action_update_user_profile', 'protalks_core_user_profile_social_update', 10, 2 );
}
