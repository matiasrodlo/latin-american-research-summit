...
class ProTalksCore_Dashboard_Import {
	/**
	 * @var ProTalksCore_Dashboard_Import of current class
	 */
	...
	/**
	 * @return ProTalksCore_Dashboard_Import
	 */
	...
	public function set_import_url() {
		$params = ProTalksCore_Dashboard::get_instance()->get_import_params();
				...
				switch ( $_POST['options']['action'] ) :
					...
					case 'rev-slider':
						if ( qode_framework_is_installed( 'revolution-slider' ) ) {
							...
							$this->set_message( esc_html__( 'Revolution Slider isn\'t installed', 'protalks-core' ) );
						}
						break;
					case 'content':
						...
						if ( $content_start ) {
							if ( ! ProTalksCore_Dashboard::get_instance()->check_purchase_code( $_POST['options']['demo'] ) ) {
								qode_framework_get_ajax_status( 'error', esc_html__( 'Please don\'t try to hack me. Purchase code registered is not valid', 'protalks-core' ) );
								exit;
							}
						}
	...
	public function file_content( $path ) {
		...
		if ( 200 !== intval( $response_code ) ) {
			$this->set_message( $response['response']['message'] . ' ' . esc_html__( 'Please contact support', 'protalks-core' ) );
			$this->set_status( 'error' );
	...
	public function import_widgets( $demo ) {
		...
		if ( $widgets_content ) {
			foreach ( (array) $widgets_content['widgets'] as $protalks_widget_id => $protalks_widget_data ) {
				$protalks_widget_data = $this->fix_nav_menu_widgets( $protalks_widget_data, $protalks_widget_id );
				update_option( 'widget_' . $protalks_widget_id, $protalks_widget_data );
			}

			$ws = $this->import_sidebars_widgets( $widgets );

			if ( $ws ) {
				$this->set_message( esc_html__( 'Widgets are set for proper sidebar', 'protalks-core' ) );
				$this->set_data( 'type', 'options' );
				$this->set_status( 'success' );
			}
	...
	public function import_sidebars_widgets( $file ) {
		$protalks_sidebars = get_option( 'sidebars_widgets' );
		unset( $protalks_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$protalks_sidebars = array_merge( (array) $protalks_sidebars, (array) $data['sidebars'] );
			unset( $protalks_sidebars['wp_inactive_widgets'] );
			$protalks_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $protalks_sidebars );
			$protalks_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $protalks_sidebars );
	...
	public function import_options( $file ) {
		...
		$current_options = get_option( PROTALKS_CORE_OPTIONS_NAME );

		if ( $options ) {
			if ( $current_options != $options ) {
				$result = update_option( PROTALKS_CORE_OPTIONS_NAME, $options );

				if ( $result ) {
					$this->update_options_after_import( $file );
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Options imported successfully', 'protalks-core' ) );

					$this->update_options_after_import( $file );
					$this->update_qi_blocks_options( $file );

				} else {
					$this->set_status( 'error' );
					$this->set_message( esc_html__( 'Problem occurred during options import', 'protalks-core' ) );
				}
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Options are already imported', 'protalks-core' ) );
			}
		}
	}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids       = get_transient( '_protalks_core_imported_posts' );
		$fields_status = true;
		...
		if ( $pages ) {
			...
			if ( ! $fields_status ) {
				$this->set_status( 'error' );
				$this->set_message( esc_html__( 'Problem occurred during settings pages import', 'protalks-core' ) );
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Settings pages imported successfully', 'protalks-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'File doesn\'t exist', 'protalks-core' ) );
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if ( $menus_data ) {
			...
			$this->set_status( 'success' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Menus set for proper locations', 'protalks-core' ) );
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'Problem during menus location set', 'protalks-core' ) );
		}
	}

	public function import_content( $file, $xml, $attachments, $post_id ) {
		ob_start();
		require_once( PROTALKS_CORE_INC_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
		...
		$protalks_import = new WP_Import();
		set_time_limit( 0 );

		$protalks_import->fetch_attachments = $attachments;
		$returned_value                     = $protalks_import->import( $file . $xml );

		if ( is_wp_error( $returned_value ) ) {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'xml', $xml );
			$this->set_message( esc_html__( 'An error occurred during content import', 'protalks-core' ) );
		} else {
			$this->set_status( 'success' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'posts', $this->imported_posts );
			$this->set_message( esc_html__( 'File imported successfully', 'protalks-core' ) . ' ' . $xml );
		}
		...
		if ( $files_created ) {
			...
			if ( class_exists( 'RevSliderSliderImport' ) ) {
				...
				if ( ! empty( $import_instance ) && ! is_wp_error( $import_instance ) ) {
					...
					foreach ( $demo_rev_sliders as $demo_rev_slider ) {
						...
						if ( ! in_array( str_replace( '.zip', '', $demo_rev_slider ), $rev_sliders, true ) ) {
							...
							if ( ! $rev_slider_result['success'] ) {
								$this->set_status( 'error' );
								$this->set_message( esc_html__( 'Error while importing rev sliders', 'protalks-core' ) );
								exit;
							}
						}
					}
					...
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Rev sliders imported successfully', 'protalks-core' ) );
				} else {
					$this->set_status( 'error' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'RevSliderSliderImport instance is empty', 'protalks-core' ) );
				}
			} else {
				$this->set_status( 'error' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'RevSliderSliderImport class doesn\'t exist', 'protalks-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Files don\'t exist', 'protalks-core' ) );
		}
	}
	...
	public function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( PROTALKS_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( PROTALKS_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	
	public function update_qi_blocks_options( $file ) {
		$qi_blocks_options_file    = $file . 'qi-blocks-options.txt';
		$qi_blocks_options         = $this->unserialized_content( $qi_blocks_options_file );
		$current_qi_blocks_options = get_option( 'qi_blocks_global_styles' );
		
		if ( $qi_blocks_options && ( $current_qi_blocks_options !== $qi_blocks_options ) ) {
			update_option( 'qi_blocks_global_styles', $qi_blocks_options );
		}
	}

	public function update_widgets_after_import( $folder ) {
	...
	public function proccess_wc_attributes( $posts ) {
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && ! empty( $_POST ) && ! empty( $_POST['options']['demo'] ) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
				...
				$html = protalks_core_get_template_part( 'core-dashboard/sub-pages/import/templates', 'pages-list', '', array( 'pages' => $pages ) );

				if ( $pages ) {
					qode_framework_get_ajax_status( 'success', '', $html );
				} else {
					qode_framework_get_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'protalks-core' ), '' );
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = ProTalksCore_Dashboard_System_Info_Page::get_instance()->get_system_info();
...
ProTalksCore_Dashboard_Import::get_instance();
