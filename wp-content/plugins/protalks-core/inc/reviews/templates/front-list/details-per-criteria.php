<?php if ( is_array( $post_ratings ) && count( $post_ratings ) ) { ?>
	<?php $average_rating_total = protalks_core_get_total_average_rating( $post_ratings ); ?>
					...
					<div class="qodef-grid-item qodef-col--content">
						<div class="qodef-item-reviews-average-wrapper">
							<div class="qodef-item-reviews-average-rating">
								<?php echo esc_html( protalks_core_reviews_format_rating_output( $average_rating_total ) ); ?>
							</div>
							<div class="qodef-item-reviews-verbal-description">
								<span class="qodef-item-reviews-rating-icon">
									<?php echo protalks_core_reviews_get_icon_for_rating( $average_rating_total ); ?>
								</span>
								<span class="qodef-item-reviews-rating-description">
									<?php echo esc_html( protalks_core_reviews_get_description_for_rating( $average_rating_total ) ); ?>
								</span>
							</div>
						</div>
					</div>
					<div class="qodef-grid-item qodef-col--sidebar">
						<div class="qodef-rating-percentage-wrapper">
							...
							foreach ( $post_ratings as $rating ) {
								$percentage = protalks_core_post_average_rating_per_criteria( $rating );
								if ( 0 == $percentage ) {
									$percentage_alt = 0.0001;
								} else {
									$percentage_alt = $percentage;
								}
								echo do_shortcode( '[protalks_core_progress_bar layout="line" number="' . esc_attr( $percentage_alt ) . '" title="' . esc_attr( $rating['label'] ) . '"]' );
							}
							?>
						</div>
					</div>
