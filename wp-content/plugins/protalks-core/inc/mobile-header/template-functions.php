...
if ( ! function_exists( 'protalks_core_get_mobile_header_logo_image' ) ) {
	/**
	 * Function that return logo image html for current module
	 *
	 * @return string that contains html content
	 */
	function protalks_core_get_mobile_header_logo_image() {
		$logo_height     = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_height' );
		$logo_source     = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_source' );
		$main_logo_id    = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_main' );
		$logo_svg_path   = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_svg_path' );
		$logo_text       = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_text' );
		...
		$available_logos = apply_filters(
			'protalks_core_filter_available_mobile_header_logo_images',
			array(
				'main' => 'main',
			),
			$parameters
		...
		if ( 'svg-path' === $logo_source && ! empty( $logo_svg_path ) ) {
			$logo_html['logo_main_image'] = apply_filters( 'protalks_core_filter_mobile_header_logo_svg_path', $logo_svg_path, $parameters );

			$is_enabled = true;
		} elseif ( 'textual' === $logo_source && ! empty( $logo_text ) ) {
			$logo_html['logo_main_image'] = esc_html( apply_filters( 'protalks_core_filter_mobile_header_logo_textual', $logo_text, $parameters ) );
			...
			foreach ( $available_logos as $logo_key => $option_value ) {
				...
				$logo_image_id = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_' . $option_value );
		...
		$parameters['logo_image'] = implode( '', apply_filters( 'protalks_core_filter_mobile_header_logo_image_html', $logo_html, $parameters ) );

		if ( $is_enabled ) {
			echo apply_filters( 'protalks_core_filter_get_mobile_header_logo_image', protalks_core_get_template_part( 'mobile-header/templates', 'parts/mobile-logo', '', $parameters ), $parameters, $logo_html );
		}
	}
}

if ( ! function_exists( 'protalks_core_set_mobile_header_logo_styles' ) ) {
	/**
	 ...
	 * @return string
	 */
	function protalks_core_set_mobile_header_logo_styles( $style ) {
		$logo_styles  = array();
		$logo_height  = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_height' );
		$logo_padding = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_padding' );
		...
		// Logo SVG Source.
		$svg_styles     = array();
		$svg_icon_color = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_svg_path_color' );
		...
		$svg_icon_size   = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_svg_path_size' );
		...
		$svg_icon_hover_color = protalks_core_get_post_value_through_levels( 'qodef_mobile_logo_svg_path_hover_color' );
		...
		// Logo Textual Source.
		$textual_styles       = protalks_core_get_typography_styles( 'qodef_mobile_logo_text', '', null );
		$textual_hover_styles = protalks_core_get_typography_hover_styles( 'qodef_mobile_logo_text', null );
	...
	add_filter( 'protalks_filter_add_inline_style', 'protalks_core_set_mobile_header_logo_styles' );
}
