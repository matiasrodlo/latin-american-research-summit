...
class Qode_Optimizer_Framework_Options {
	 ...
	 *
	 * @return Qode_Optimizer_Framework_Options
	 */
	...
	public function set_child_element( Qode_Optimizer_Framework_Page $page ) {
		$key                          = $page->get_slug();
	...
	public function add_option_page( Qode_Optimizer_Framework_Page $page ) {
	...
	public function enqueue_dashboard_framework_scripts( $exclude_scripts = array() ) {
		...
		wp_enqueue_style( 'select2', QODE_OPTIMIZER_ADMIN_URL_PATH . '/inc/common/assets/plugins/select2/select2.min.css', array(), '4.0.13' );

		// Hook to include additional scripts before dashboard scripts.
		do_action( 'qode_optimizer_action_framework_before_dashboard_scripts' );

		// Main dashboard css styles.
		if ( ! isset( $exclude_scripts['main_style'] ) ) {
			// phpcs:ignore WordPress.WP.EnqueuedResourceParameters
			wp_enqueue_style( 'qode-optimizer-framework-style', QODE_OPTIMIZER_ADMIN_URL_PATH . '/inc/common/assets/css/common.min.css' );
		}

		// Enqueue 3rd party plugins scripts.
		wp_enqueue_script( 'jquery-ui-sortable' );
		wp_enqueue_script( 'select2', QODE_OPTIMIZER_ADMIN_URL_PATH . '/inc/common/assets/plugins/select2/select2.full.min.js', array(), '4.0.13', true );
		...
		do_action( 'qode_optimizer_action_framework_after_dashboard_scripts' );
	}
}

if ( ! function_exists( 'qode_optimizer_framework_options' ) ) {
	/**
	 * Function that initialize main framework options object
	 */
	function qode_optimizer_framework_options() {
		return Qode_Optimizer_Framework_Options::get_instance();
	}
}
