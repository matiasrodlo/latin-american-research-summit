...
if ( isset( $enable_filter ) && 'yes' === $enable_filter ) {
	$filter_items = protalks_get_filter_items( $params );
	?>
		...
		<?php if ( ! empty( $filter_items ) ) { ?>
			<div class="qodef-m-filter-items">
				<a class="qodef-m-filter-item qodef--active" href="#" data-taxonomy="<?php echo esc_attr( $taxonomy_filter ); ?>" data-filter="*">
					<span class="qodef-m-filter-item-name"><?php esc_html_e( 'Show All', 'protalks' ); ?></span>
				</a>
				<?php
	...
	if ( ! isset( $pagination_type ) || 'no-pagination' === $pagination_type ) {
		protalks_render_svg_icon( 'spinner', 'qodef-filter-pagination-spinner' );
	}
	?>
<?php } ?>
